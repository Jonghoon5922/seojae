...
`search` 응답에는 항상 `term_document_counts`가 실린다. `hint`는 셋 중 하나일 때만 붙는다.

1. 결과가 없다
2. 문서 수 0인 검색어가 있다 (이 서재가 쓰지 않는 말)
3. 검색어가 전부 문서의 25% 이상에 나온다 (변별력 없음. 문서 20건 미만이면 이 규칙은 적용 안 함 —
   3건 중 1건이 이미 33%라 비율이 의미가 없다)

bxm-guide 576문서 실측 (흔한 말 기준 = 144문서):

| 질문 | 검색어별 문서 수 | hint | 판정 |
|---|---|---|---|
| 예외 처리는 어떻게 해야 하나 | 예외 269, 처리 389, 어떻 31 | 없음 | 맞음 (결과도 정확) |
| 데이터베이스 접근 계층 | **데이터베이스 0**, 접근 28, 계층 258 | 붙음 | 맞음 |
| 리프레시 데이 정책 | 리프레시 0, 데이 0, 정책 19 | 붙음 | 맞음 |
| 에러 났을 때 어떻게 처리하지 | 에러 27, 나 46, 어떻 31, 처리 389 | 없음 | **놓침** |

**마지막 줄이 이 장치의 한계다.** "에러"는 27개 문서에 실제로 있다 — 색인에 없는 말도, 흔한 말도
아니다. 단어는 있는데 엉뚱한 문서에 있는 상황이라 규칙으로는 잡히지 않는다. 남는 대책은 둘:

- `term_document_counts`를 항상 노출해 Claude가 스스로 판단하게 한다 (지금 방식)
