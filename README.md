...
`_미분류\`에 파일을 던져두고 "정리해줘"라고 하면 Claude가 내용을 읽고 책장에 꽂는다.
책장 설명(`README.md`)도 Claude가 쓴다.

```
_미분류\20260907_회의록.md
...
│            │      │      │        │ 질문에 사용.                                    │
│ _미분류     │    1 │    1 │ 자동   │ 아직 어느 책장에도 꽂히지 않은 파일.            │
...
자동으로 들어간다. JSON을 직접 고칠 필요가 없다. Claude Desktop을 껐다 켜면 붙는다.

```
시작 메뉴 → 서재          앱 창 (검색·책장·미분류·기록·설정)
Claude Desktop            같은 서재를 도구 9개로 사용
```

둘이 **같은 설정을 보고 같은 서재를 연다.** 앱의 설정 화면에서 서재 폴더를 옮기면
Claude가 보는 서재도 따라 옮겨진다.

제거하면 등록도 같이 지워진다. 서재 폴더(내 문서)는 건드리지 않는다.

> 등록을 나중에 하거나 되돌리려면:
> `"C:\...\Seojae\seojae-mcp.exe" register` / `unregister`
> 다른 MCP 서버 설정은 건드리지 않고, 고치기 전에 `.bak` 백업을 남긴다.

### Claude Code — 설정 세 줄

파이썬이 있는 환경이면 `.mcp.json` 에 이렇게 넣는다.
